...
import { User, Mail, Shield, Building, Phone, Save, KeyRound, CheckCircle2 } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-800 rounded-sm p-4 space-y-4 shadow-sm text-xs text-[#1A1A1A] dark:text-[#F9FAFB] transition-colors duration-200">
      <div className="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm uppercase tracking-wider text-gray-900 dark:text-white">User Settings & Administrator Profile</h3>
        </div>
        <span className="px-2 py-0.5 rounded-sm bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-400 font-mono text-[10px] font-bold border border-blue-200 dark:border-blue-800">
          {userProfile.role}
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/60 p-3.5 rounded-sm border border-gray-200 dark:border-gray-700">
        <img
          ...
          className="w-12 h-12 rounded-full border border-gray-300 dark:border-gray-700 object-cover shrink-0"
        />
        <div>
          <h4 className="font-bold text-sm text-gray-900 dark:text-white">{userProfile.name}</h4>
          <p className="text-gray-500 dark:text-gray-400 text-xs">{userProfile.email}</p>
          <div className="flex items-center gap-2 mt-1 text-[11px] text-gray-500 dark:text-gray-400">
            <span>Role: <strong className="text-gray-800 dark:text-gray-200">{userProfile.role}</strong></span>
            <span>•</span>
            <span>Last Login: <strong className="text-gray-800 dark:text-gray-200">{new Date(userProfile.lastLogin).toLocaleTimeString()}</strong></span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-gray-700 dark:text-gray-300 font-bold mb-1 flex items-center gap-1">
            <Mail className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" /> Email Address
          </label>
          <input
            type="email"
            value={email}
